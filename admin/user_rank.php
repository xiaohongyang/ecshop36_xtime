...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $_POST['year_points'] = empty($_POST['year_points']) ? 0 : intval($_POST['year_points']);
    ...
    $sql = "INSERT INTO " .$ecs->table('user_rank') ."( ".
                "rank_name, min_points, max_points, discount, special_rank, year_points, show_price".
            ") VALUES (".
                "'$_POST[rank_name]', '" .intval($_POST['min_points']). "', '" .intval($_POST['max_points']). "', ".
                "'$_POST[discount]', '$special_rank', '" .intval($_POST['year_points']). "', '" .intval($_POST['show_price']). "')";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit_year')
{
    ...
    if ($exc->edit("year_points = '$val'", $rank_id))
    {
        $rank_name = $exc->get_name($rank_id);
        admin_log(addslashes($rank_name), 'edit', 'user_rank');
        clear_cache_files();
