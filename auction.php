...
if ($_REQUEST['act'] == 'list')
{
    ...
    if ($count > 0)
    {
        ...
        $sort = \zd\Helper::get('sort');
        $order = \zd\Helper::get('order');
    ...
    if (!$smarty->is_cached('auction_list.dwt', $cache_id))
    {
        if ($count > 0)
        {
            ...
            if ($sort == 'process') {
                ...
                usort($auction_list, function ($a, $b) use ($order, $time) {
                    $diff1 = abs($a['end_time'] - $time);
                    $diff2 = abs($b['end_time'] - $time);
                    if ($diff1 == $diff2) {
                        return 0;
                    }
                    if ($order != 'asc') {
            ...
            elseif ($sort == 'time') {
                ...
                usort($auction_list, function ($a, $b) {
                    if ($a['is_finished']) {
                        return -1;
                    }
                    if ($b['is_finished']) {
...
elseif ($_REQUEST['act'] == 'bid')
{
    ...
    if (!\Zodream\Infrastructure\Http\RequestFinal::isPost()) {
        ...
        if ($last_log) {
            $smarty->assign('last_price', $last_log['price']);

        }
        if (isset($_GET['address'])) {
            $_SESSION['auction_address'] = \zd\UserAddress::getRegion(intval($_GET['address']));
        }
        if (!isset($_SESSION['auction_address']) && $last_log) {
            $_SESSION['auction_address'] = \zd\UserAddress::getRegion($last_log['address_id']);
        }
        $smarty->assign('address', $_SESSION['auction_address']);
        $smarty->display('auction_bid.dwt');
        exit();
    }
    ...
    $address_id = intval(\zd\Helper::post('address_id'));
    if ($address_id < 1) {
        show_message('请选择收货地址', '', '', 'error');
    }
