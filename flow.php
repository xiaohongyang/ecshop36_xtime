...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    // $allGood = $db->getAll($sql);
    // var_dump($allGood);exit;
    // if (count($allGood) == 0)
    // {
    //     show_message($_LANG['no_goods_in_cart'], '', '', 'warning');
    // }
    // foreach ($allGood as $key => $value) {
    //     $sql = "SELECT goods_name,goods_number,is_on_sale,is_alone_sale,is_delete FROM " . $ecs->table('goods') . " WHERE goods_id = " . $value['goods_id'];
    //     $goods = $db->getRow($sql);
    //     if ($goods['goods_number'] == 0) {
    //         show_message($goods['goods_name'].' 库存不足', '', '', 'warning');
    //     }
    //     if ($goods['is_on_sale'] != 1 || $goods['is_alone_sale'] != 1 || $goods['is_delete'] != 0) {
    //         show_message($goods['goods_name'].' 商品已下架', '', '', 'warning');
    //     }
    // }
    ...
    $consignee = null;
    $address_id = intval(Helper::request('address'));
    if ($address_id > 0) {
        $consignee = \zd\UserAddress::get($address_id);
    }
    if (empty($consignee)) {
        ...
        foreach ($payment_list as $key => $payment)
        {
            ...
            if ($payment['pay_code'] == 'balance')
            {
                ...
                else
                {
                    ...
                    $money = Sql::create()->select('user_money')
                        ->from('users')->where('user_id='.$_SESSION['user_id'])->scalar();
                    $payment_list[$key]['pay_name'] = $payment['pay_name'].'(余额：'.
                        $money.')';
                    $payment_list[$key]['has_money'] = $money >= $order['order_amount'];
                }
            }
        ...
        && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS))
    {
        ...
        $points  = flow_available_points();
        $smarty->assign('order_max_integral', $points[1]);  // 可用积分
        $smarty->assign('order_max_integral_money', $points[0]);
...
elseif ($_REQUEST['step'] == 'change_integral')
{
    ...
    $flow_points = flow_available_points()[1];  // 该订单允许使用的积分
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    /* 订单中的商品 */
    $cart_goods = Cart::getSelected($flow_type);
    ...
    /* 插入订单商品 */
    Cart::toOrder($new_order_id, $flow_type);
    /* 修改拍卖活动状态 */
...
$smarty->assign('menuIndex',    2);
assign_dynamic('shopping_flow');

$smarty->display('flow_'.$_REQUEST['step'].'.dwt');
