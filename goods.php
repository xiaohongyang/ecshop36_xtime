...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
        ...
        $smarty->assign('goods_list',       get_recommend_goods('hot'));     // 热销商品

        // 标签获取
        $sqlTag = 'SELECT tag_words FROM ' . $ecs->table('tag') . " WHERE goods_id={$goods['goods_id']}";
        $resTag = $db->query($sqlTag);
        while ($rowTag = $db->FetchRow($resTag))
        {
            $tag_words[] = $rowTag['tag_words'];
        }

        $smarty->assign('tag_words', $tag_words);
