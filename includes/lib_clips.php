...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT pay_points, user_money, avatar, nick_name, credit_line, last_login, is_validated FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
    ...
    $info['nick_name'] = empty($row['nick_name']) ? $info['username'] : $row['nick_name'];
