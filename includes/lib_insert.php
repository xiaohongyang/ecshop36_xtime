 ...
 * @return string
 */
function insert_page($arr) {
    if (!array_key_exists('size', $arr)) {
        $arr['size'] = 20;
    }
    return \zd\Helper::page($arr['total'], $arr['size']);
}
...
function insert_sales($args) {
    global $smarty;
    $time = 7 * 24 * 3600;
    if (isset($args['year'])) {
        $time = intval($args['year']) * 365 * 24 * 3600;
    } elseif (isset($args['month'])) {
        $time = intval($args['month']) * 30 * 24 * 3600;
    } elseif (isset($args['week'])) {
        $time = intval($args['week']) * 7 * 24 * 3600;
    }  elseif (isset($args['day'])) {
        $time = intval($args['day']) * 24 * 3600;
    }  elseif (isset($args['hour'])) {
        $time = intval($args['hour']) * 3600;
    }
    $num = isset($args['num']) ? intval($args['num']) : 6;
    $time = gmtime() - $time;
    $goods_list = \zd\Sql::create()
        ->select('og.goods_id, g.goods_name, g.shop_price, g.goods_thumb, SUM(og.goods_number) as total')
        ->from('order_goods og')
        ->left('order_info oi', 'og.order_id = oi.order_id')
        ->left('goods g', 'og.goods_id = g.goods_id')
        ->where('oi.pay_time > '. $time)
        ->group('og.goods_id')
        ->order('total desc')->limit($num)->all();
    foreach ($goods_list as &$item) {
        $item['url'] = 'goods.php?id='. $item['goods_id'];
        $item['shop_price'] = price_format($item['shop_price'], false);
    }
    $smarty->assign('goods_list', $goods_list);
    return $smarty->fetch('library/goods_list.lbi');
}
 ...
 * @param $arr
 */
function insert_auction($arr) {
    $id = intval($arr['id']);
    if ($id < 1) {
        return '未出价';
    }
    $price = \zd\Sql::create()->select('Max(bid_price) as price')
        ->from('auction_log')
        ->where('act_id = '.$id)->scalar();
    return empty($price) ? '未出价' : price_format($price, false);
}

function insert_auction_btn($arr) {
    global $smarty;
    $auction = $arr['auction'];
    $price = 0;
    $status = 0;
    ...
    if (isset($_SESSION['user_id'])) {
        $price = \zd\Sql::create()
            ->select('Max(bid_price) as price')
            ->from('auction_log')
            ->where('act_id = '.$auction['act_id'])
            ->andWhere('bid_user='.$_SESSION['user_id'])
            ->scalar();
        $maxprice = \zd\Sql::create()
            ->select('Max(bid_price) as price, bid_user')
            ->from('auction_log')
            ->where('act_id = '.$auction['act_id'])->one();
        if (!empty($maxprice) && $_SESSION['user_id'] === $maxprice['bid_user']) {
            $status = 2;
        }
    }
    ...
    return $smarty->fetch('library/auction_btn.lbi');
}
