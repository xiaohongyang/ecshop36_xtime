...
{
    $sql = 'SELECT pay_id, pay_code, ico, pay_name, pay_fee, pay_desc, pay_config, is_cod' .
            ' FROM ' . $GLOBALS['ecs']->table('payment') .
            ' WHERE enabled = 1 ';
    if (!$support_cod)
    {
...
//            $shipping_count = $GLOBALS['db']->getOne($sql);
            ...
            $total['shipping_fee'] = ($is_shipping AND $weight_price['free_shipping'] == 1) ?0 :  shipping_fee($shipping_info['shipping_code'],$shipping_info['configure'], $weight_price['weight'], $total['goods_price'], $weight_price['number']);
