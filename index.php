...
/*------------------------------------------------------ */
/* 缓存编号 */
$filter = \zd\Helper::get('filter');
$sort = \zd\Helper::get('sort', 'price');
$order = \zd\Helper::get('order', $sort == 'time' ? 'desc' : 'asc');
$sorts = array('total','sale','price','time');
...
if (!$smarty->is_cached('index.dwt', $cache_id))
{
    ...
    $where = '';
    if ($filter == 'zk') {
        $where .= ' AND g.promote_price > 0';
    } elseif ($filter == 'xg') {
        $where .= ' AND g.goods_number > 1';
    } elseif ($filter == 'by') {
        $where .= ' AND g.is_shipping > 0';
    } elseif ($filter == 'hy') {
        $where .= ' AND g.goods_id > 0';
    }
    $smarty->assign('filter', $filter);

    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.add_time, g.click_count, g.shop_price AS org_price, ' .
    ...
    foreach ($res AS $idx => $row)
    {
        ...
        $goods[$idx]['market_price'] = price_format($row['market_price'], false);
    ...
    function total_asc($a,$b){
        if ($a['total_sort']==$b['total_sort']) return 0;
        return ($a['total_sort']<$b['total_sort'])?-1:1;
    }
    function total_desc($a,$b){
        if ($a['total_sort']==$b['total_sort']) return 0;
        return ($a['total_sort']>$b['total_sort'])?-1:1;
    }

    function sale_asc($a,$b){
        if ($a['cum_sales']==$b['cum_sales']) return 0;
        return ($a['cum_sales']<$b['cum_sales'])?-1:1;
    }
    function sale_desc($a,$b){
        if ($a['cum_sales']==$b['cum_sales']) return 0;
        return ($a['cum_sales']>$b['cum_sales'])?-1:1;
    }

    function price_asc($a,$b){
        if ($a['sort_price']==$b['sort_price']) return 0;
