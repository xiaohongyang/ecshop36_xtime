...
$not_login_arr =
array('login','act_login','register','act_register',
    'oath', 'oath_login',
'has_collect',
'act_edit_password','get_password','send_pwd_email',
'password', 'signin', 'add_tag', 'collect', 'return_to_cart',
'logout', 'email_list', 'validate_email',
'send_hash_mail', 'order_query', 'is_registered',
'check_email','clear_history','qpassword_name',
'get_passwd_question', 'check_answer');

/* 显示页面的action列表 */
$ui_arr = array('register', 'login', 'profile', 'order_list', 'order_detail',
'address_list', 'address',  'collection_list', 'phone1', 'phone2',
'message_list', 'tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply',
'account_deposit', 'account_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list','validate_email','track_packages', 'transform_points','qpassword_name', 'get_passwd_question', 'check_answer','delivery_info','edit_nickname','address_choose','address_detail','my_card','my_score','my_account');
...
if ($action == 'default')
{
    ...
    $smarty->assign('page_title', '我的');
    $smarty->display('user.dwt');
}

if ($action == 'edit_nickname') {
    // var_dump($action);exit;
    $smarty->assign('name', Sql::create()
        ->select('nick_name')
    ->from('users')->where('user_id='.$user_id)->scalar());
    $smarty->assign('rightBtn', [
       'label' => '保存'
    ]);
    $smarty->display('user_name.dwt');
}

if ($action == 'address_choose') {
    // var_dump($action);exit;
    $smarty->display('user_clips.dwt');
}

if ($action == 'address_detail') {
    // var_dump($action);exit;
    $smarty->display('user_clips.dwt');
}
...
if ($action == 'my_score') {
    ...
    $smarty->assign('info', User::profile());
    $smarty->assign('rank_list', UserRank::specialRank());
    $smarty->display('user_score.dwt');
}

if ($action == 'buy_rank') {
    $rank = intval(Helper::request('rank'));
    if (empty($rank)) {
        Helper::failure('等级不存在！');
    }
    if (UserRank::buyRank($rank)) {
        Helper::success();
    }
...
if ($action == 'my_account') {
    // var_dump($action);exit;
    $smarty->assign('info', Sql::create()
    ->from('users')->where('user_id='.$user_id)->one());
    $smarty->assign('page_title', '我的账户');
...
if ($action == 'register')
{
    ...
    // 手机短信验证码
    $smarty->assign('send_code', $send_code);
    $smarty->assign('page_title', '注册');
...
elseif ($action == 'act_register')
{
    ...
    else
    {
        ...
        if (empty($username)) {
            show_message('手机号码不可为空', $_LANG['sign_up'], 'user.php?act=register', 'error');
        }
        if (empty($mobile_code)) {
            show_message('手机验证码不可为空', $_LANG['sign_up'], 'user.php?act=register', 'error');
        }
        if($username != $_SESSION["sms_mobile"]){
            show_message('请填写发送短信时的手机号码', $_LANG['sign_up'], 'user.php?act=register', 'error');
        }
        if ($_POST['mobile_code'] != $_SESSION["sms_mobile_code"]) {
            show_message('手机验证码输入错误', $_LANG['sign_up'], 'user.php?act=register', 'error');
        }
...
elseif ($action == 'get_password')
{
    ...
    else
    {
        ...
        $send_code = StringExpand::random();
        $_SESSION['send_code'] = $send_code;
        $smarty->assign('send_code', $send_code);
        $smarty->assign('page_title', '找回密码');
        //显示用户名和email表单
        $smarty->display('user_password1.dwt');
    }
}
...
elseif ($action == 'phone1') {
    ...
    //显示重置密码的表单
    $smarty->display('user_phone1.dwt');
}
elseif ($action == 'phone2') {
    $smarty->assign('rightBtn', [
        'label' => '下一步'
    ]);
    $send_code = StringExpand::random();
    ...
    $smarty->assign('page_title', '绑定手机');
    //显示重置密码的表单
    $smarty->display('user_phone2.dwt');
}
elseif ($action == 'check_pwd') {
    $password = Helper::post('password');
    if (!$user->check_user($_SESSION['user_name'], $password)) {
        Helper::failure('密码错误！');
    }
    Helper::success();
}
elseif ($action == 'act_edit_phone') {
    $old_code = Helper::post('old_phone');
    $password = Helper::post('password');
    if (empty($old_code)) {
        if (!$user->check_user($_SESSION['user_name'], $password)) {
            show_message('密码错误！');
        ...
        if ($_SESSION['user_name'] != $_SESSION["sms_mobile1"]
            || $old_code != $_SESSION["sms_mobile_code1"]) {
            show_message('验证码错误！');
        }
    }
    $code = Helper::post('code');
    $phone = Helper::post('phone');
    if ($code == $_SESSION["sms_mobile_code"] && $_SESSION["sms_mobile"] == $phone) {
        $u = Sql::create()
...
elseif ($action == 'reset_password')
{
    ...
    $_SESSION['send_code'] = $send_code;
    $smarty->assign('send_code', $send_code);
    $smarty->assign('page_title', '密码修改');
    $smarty->assign('info', Sql::create()
    ->from('users')->where('user_id='.$user_id)->one());
    //显示重置密码的表单
    $smarty->display('user_password.dwt');
}
...
elseif ($action == 'act_edit_password')
{
    ...
    if (isset($_SESSION['user_id']) && !empty($_SESSION['user_id'])) {
        if ($_SESSION["sms_mobile"] === $_SESSION['user_name']
            ...
            if (Sql::update('users', [
                'password' => $user->compile_password($info)
            ], 'user_id='.$user_id)) {
                $user->logout();
    ...
    if (($user_info && (!empty($code) && md5($user_info['user_id'] .
                    $_CFG['hash_code'] . $user_info['reg_time']) == $code))
        || ($_SESSION['user_id']>0
            && $_SESSION['user_id'] == $user_id
            && $user->check_user($_SESSION['user_name'], $old_password)))
    {
		
        if ($user->edit_user(array('username'=> (empty($code) ? $_SESSION['user_name'] : $user_info['user_name']),
            'old_password'=>$old_password,
            'password'=>$new_password), empty($code) ? 0 : 1))
        {
...
elseif ($action == 'order_list')
{
    ...
    $pager = \zd\UserOrder::getPage($user_id);
    $merge  = get_user_merge($user_id);

    include_once(ROOT_PATH . 'includes/cls_certificate.php');
    $cert = new certificate();
    $cert->is_bind_sn('erp','goods_name')?$smarty->assign('no_bind_erp',  0):$smarty->assign('no_bind_erp',  1);
    $type_title = ['我的订单', '待付款的订单', '待收货的订单', '已完成的订单'];
    $type = Helper::get('type', 0);
    $smarty->assign('page_title', isset($type_title[$type]) ? $type_title[$type]: '我的订单');

    $smarty->assign('open_logistics_trace',is_open_logistics_trace());
    $smarty->assign('merge',  $merge);
    $smarty->assign('type',  $type);
    $smarty->assign('pager',  $pager->getLink());
    $smarty->assign('backUrl',      'user.php');




    $smarty->assign('order_list', $pager->getPage());
...
elseif ($action == 'order_detail')
{
    ...
    /* 订单商品 */
    $goods_list = \zd\UserOrder::getOrderGoods($order_id);
    ...
    $order['format_status'] = \zd\UserOrder::getStatusLabel($order);
    /* 订单 支付 配送 状态语言项 */
    //$order['order_status'] = $_LANG['os'][$order['order_status']];
   // $order['pay_status'] = $_LANG['ps'][$order['pay_status']];
    //$order['shipping_status'] = $_LANG['ss'][$order['shipping_status']];
    $order['region'] = Sql::create()
        ->select('concat(IFNULL(p.region_name, \'\'), \'  \', IFNULL(t.region_name, \'\'), \'  \', IFNULL(d.region_name, \'\')) AS region')
    ->from('order_info oi')
        ->left('region p', 'oi.province = p.region_id')
        ->left('region t', 'oi.city = t.region_id')
        ->left('region d', 'oi.district = d.region_id')
    ->where('oi.order_id =' .$order_id)->scalar();

    $smarty->assign('order',      $order);
    
    $smarty->assign('backUrl',      'user.php?act=order_list');
...
elseif ($action == 'address_list')
{
    ...
    /* 获得用户所有的收货人信息 */
    $consignee_list = UserAddress::all();
    ...
    if ($mode == 'select') {
        $_SESSION['backUrl'] = \Zodream\Service\Routing\Url::referrer();
    }

    $smarty->assign('rightBtn', $mode == 'select' ? [
        'url' => 'user.php?act=address_list',
        'label' => '地址管理'
    ]  : [
        'url' => 'user.php?act=address',
        'label' => '新增'
    ]);
    ...
    if (isset($_SESSION['backUrl'])) {
        ...
        $smarty->assign('backUrl', $mode == 'select' ? \Zodream\Service\Routing\Url::referrer() : 'user.php');    
    }
...
elseif ($action == 'address') {
    $id = intval(Helper::get('id'));
    if ($id > 0) {
        $smarty->assign('page_title', '编辑收货地址');
        $smarty->assign('consignee', \zd\UserAddress::get($id));
    } else {
        $smarty->assign('page_title', '新增收货地址');
    }
    $smarty->assign('rightBtn', [
        'label' => '保存'
    ]);
...
elseif ($action == 'act_info') {
    ...
    if (!in_array($name, ['avatar', 'nick_name', 'birthday', 'sex'])) {
        Helper::failure('参数错误！');
    }
    Sql::update('users', [
...
elseif ($action == 'act_edit_address')
{
    ...
    $id = UserAddress::save($address, !empty(Helper::post('default')));
    if (!empty($id)) {
        Helper::success(UserAddress::getRegion($id));
    }
    Helper::failure('保存失败！'.Sql::getError());
}
...
elseif ($action == 'drop_consignee')
{
    ...
    {
        Helper::failure($_LANG['del_address_false']);
    }
}
...
elseif ($action == 'collect') {
    ...
    if (!isset($_SESSION['user_id']) || $_SESSION['user_id'] == 0) {
        Helper::failure($_LANG['login_please']);
    }
    /* 检查是否已经存在于用户的收藏夹 */
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('collect_goods') .
        " WHERE user_id='$_SESSION[user_id]' AND goods_id = '$goods_id'";
    if ($GLOBALS['db']->GetOne($sql) > 0) {
        Helper::failure($GLOBALS['_LANG']['collect_existed']);
    }
    $time = gmtime();
    ...
    if ($GLOBALS['db']->query($sql) === false) {
        Helper::failure($GLOBALS['db']->errorMsg());
    }
    Helper::success($GLOBALS['_LANG']['collect_success']);
}
